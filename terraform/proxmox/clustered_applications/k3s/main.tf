module "k3s_server_node" {
    ...
    remote_exec_script = replace(replace(replace(replace(file("${path.module}/install-k3s-server.sh"), "k3s_token", var.k3s_token), "k3s_cluster_ip", var.k3s_cluster_ip), "k3s_nfs_ip", var.k3s_nfs_ip), "k3s_nginx_cluster_ip", var.k3s_cluster_ip_nginx)
}
